...
Use these boxes through the generated `metadata.json` file
rather than by adding a `.box` file directly.

## Use hosted metadata

If the publisher generated metadata with `BOX_BASE_URL`,
add the box from the hosted `metadata.json` file:
...
Hosted metadata is the right choice
when the box artifacts are shared with other machines.

## Use local metadata

If the metadata was generated without `BOX_BASE_URL`,
each provider entry uses an absolute `file://` URL
that points at the local `.box` file.
...
This mode works when the `metadata.json` file
and the referenced `.box` files are available on the same machine
at the paths recorded in the metadata.
