...
This repository builds Ubuntu Vagrant base boxes for the `libvirt` and `virtualbox` providers.
...
By default this builds Ubuntu 22.04 and 24.04 boxes for both supported providers and writes the published artifacts under `dist/generic/`.
...
| `PROVIDERS`    | `libvirt virtualbox`    | Comma- or space-separated list of providers to build. Supported values are `libvirt` and `virtualbox`.             |
