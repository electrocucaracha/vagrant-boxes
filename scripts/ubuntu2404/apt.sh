...
retry apt-get --assume-yes -o Dpkg::Options::="--force-confnew" update
error
...
error
retry apt-get --assume-yes -o Dpkg::Options::="--force-confnew" dist-upgrade
error

# Needed to retrieve source code, and other misc system tools.
retry apt-get --assume-yes install vim vim-nox gawk git git-man liberror-perl wget curl rsync gnupg plocate sudo sysstat lsof pciutils usbutils lsb-release psmisc
error
...
# Ensure that the database is updated automatically at regular intervals, including during system startup.
